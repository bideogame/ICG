...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_TexEnable", texEnable);
		...
		VertexArrayObject::sptr coinMesh = ObjLoader::LoadFromFile("models/coin.obj");
			...
			for (int j = 0; j < y; j++) {
				...
				if (map[i][j] == 1) {
					...
					if (r == 3 && canspawn) {
						...
						coine.emplace<RendererComponent>().SetMesh(coinMesh).SetMaterial(coinMat);
		...
		GameObject bloomEffectObject = scene->CreateEntity("Bloom");
		{
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			if(bloom){
				bloomEffect->ApplyEffect(testBuffer);
